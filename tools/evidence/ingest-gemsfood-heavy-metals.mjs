...
function usage() {
  return `Usage:
  node tools/evidence/ingest-gemsfood-heavy-metals.mjs --input "/path/to/export.csv"
  node tools/evidence/ingest-gemsfood-heavy-metals.mjs --input-dir "/path/to/gems-csvs"
  node tools/evidence/ingest-gemsfood-heavy-metals.mjs --download --profile hmi

Options:
  --project-root <path>       HMI wiki project root. Defaults to HMI_WIKI_PROJECT_ROOT or the current working directory.
...
source_type: dataset
evidence_tier: A
review_state: machine_extracted_context
standards_use: context_only_until_exact_row_aggregate_review
...
The raw export files and normalized sample table live under \`raw/\`, which is intentionally gitignored. The tracked evidence layer is the source page, export manifest, grouped summary CSV, and route audit; restore or re-download the raw exports before promoting any GEMS/Food row into standards calculations.
...
function updateEvidenceReadme(root) {
  ...
  let text = read(file);
  const section = `- \`who_gemsfood_heavy_metals_summary_by_food_contaminant.csv\`: lower-bound per-food/per-contaminant summaries derived from normalized GEMS/Food exports. These rows are source-scope occurrence context and route/gap inputs until exact-row aggregate review.
- \`who_gemsfood_heavy_metals_raw_exports.csv\`: raw export manifest with access dates, row counts, and SHA-256 hashes. The raw CSV exports themselves live under gitignored \`raw/\`.
- \`who_gemsfood_arsenic_page_routes.csv\`: arsenic route audit for high-value GEMS/Food rows; entries preserve species and page-fit caveats and are not final HMTc standards values.`;
...
Notes: Preserved ${exports.length} WHO GEMS/Food CSV export file${exports.length === 1 ? "" : "s"} under gitignored \`${rawDirRel}/\`; generated ${rows.length.toLocaleString("en-US")} normalized row-level records and ${summaries.length.toLocaleString("en-US")} lower-bound summary rows. Original result text, units, LOD/LOQ, basis, QA, WHO region, food identifiers, and serial numbers are retained; source-scope percentiles remain context and no brand claims, HMTc threshold claims, or regulatory exceedance claims were added.`,
    };
