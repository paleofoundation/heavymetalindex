...
const server = http.createServer(async (request, response) => {
  try {
    ...
    response.end(error instanceof Error ? error.message : "Internal workbench error")
  }
})
...
async function buildWorkbenchContext() {
  const [products, concentrationRows, sampleRows, registerRows, standardsRows, regulationRows] = await Promise.all([
    readProductRows(),
...
async function readProductRows() {
  ...
  const files = (await readdir(productDir)).filter((file) => file.endsWith(".md") && file !== "index.md")
  const rows = []
...
async function readRegulationRows() {
  ...
  const files = (await readdir(regulationDir)).filter((file) => file.endsWith(".md") && file !== "index.md")
  const rows = []
  ...
  return rows.sort((a, b) => {
    return a.jurisdiction.localeCompare(b.jurisdiction) || a.agency.localeCompare(b.agency) || a.title.localeCompare(b.title)
  })
}

async function buildCell(row, metal, products, concentrationRows, sampleRows, registerMap, standardsMap, regulationRows, sourceTitleCache) {
  const rowRecords = concentrationRows.filter((record) => record.row_slug === row.slug && record.metal_species === metal)
  const rowP90Record = chooseBestRecord(rowRecords.filter((record) => isNumber(record.p90_ppb) && record.unit === "ppb"))
  const rowP10Record = chooseBestRecord(rowRecords.filter((record) => isNumber(record.p10_ppb) && record.unit === "ppb"))
  const standardBasis = standardBasisFor(row)
  const standardRecords = rowRecords.filter((record) => isNumber(record[standardBasis.statistic]) && record.unit === "ppb")
  ...
  const cleanRecord = chooseBestRecord(
    concentrationRows.filter(
      (record) => record.row_slug === relationship.cleanSlug && record.metal_species === metal && isNumber(record.p90_ppb),
    ),
  )
  const dirtyRecord = chooseBestRecord(
    concentrationRows.filter(
      (record) => record.row_slug === relationship.dirtySlug && record.metal_species === metal && isNumber(record.p10_ppb),
    ),
  )
  const rawSamples = sampleRows.filter((sample) => sample.row_slug === row.slug && sample.metal_species === metal)
  const register = normalizeRegisterCell(registerMap.get(cellKey(row.slug, metal)))
  const finalized = normalizeStandardDecision(standardsMap.get(cellKey(row.slug, metal)))
  const comparatorContext = comparatorFor(row, products, relationship, cleanRecord, dirtyRecord)
  ...
  const blockers = blockersFor(row, rowRecords, selected, standardBasis, cleanRecord, dirtyRecord, rawSamples, regulatoryGuardrail)
  ...
  return {
    ...
    row_standard: {
      ...
      candidate_value: selected ? numberOrNull(selected[standardBasis.statistic]) : null,
    ...
    path_a: {
      formula: "Path A candidate = selected clean-platform p90 for the same metal, product row, matrix basis, and units.",
    ...
    path_b: {
      formula: "Path B fallback = method-specific LOQ x 5 for the analyte and matrix.",
      needed: !selected,
      status: selected ? "Fallback not selected unless the row-standard statistic is rejected during review." : "LOQ package needed.",
      loq_value: null,
    ...
    evidence: {
      ...
      p90_record_count: rowRecords.filter((record) => isNumber(record.p90_ppb)).length,
      p10_record_count: rowRecords.filter((record) => isNumber(record.p10_ppb)).length,
      raw_sample_count: rawSamples.length,
      bases: [...new Set(rowRecords.map((record) => record.basis).filter(Boolean))],
...
function comparatorFor(row, products, relationship, cleanRecord, dirtyRecord) {
  const cleanRow = products.find((product) => product.slug === relationship.cleanSlug)
  const dirtyRow = products.find((product) => product.slug === relationship.dirtySlug)
  ...
  if (row.variant_type.includes("contamination_platform") && relationship.cleanSlug) {
    return {
...
function comparisonNotes(records) {
    ...
    return [
      {
        ...
        detail: "Only one extracted source row is currently mapped to this row-metal cell.",
      },
    ]
  }
  ...
  const productLabels = uniqueValues(records.map((record) => record.source_product_label || record.product_label))
  const rowFits = uniqueValues(records.map((record) => record.row_fit))
  const sampleCounts = uniqueValues(records.map((record) => record.n).filter((value) => value !== ""))
  const hasPercentileRows = records.some((record) => isNumber(record.p90_ppb) || isNumber(record.p10_ppb))
  const hasSummaryRows = records.some((record) => isNumber(record.mean_ppb) || isNumber(record.median_ppb) || isNumber(record.max_ppb))
  const hasRangeOnlyRows = records.some((record) => String(record.statistic_scope ?? "").toLowerCase().includes("range"))
  const needsQa = records.some((record) => /conflict|ambiguous|not row-split|qa/i.test(`${record.product_label} ${record.source_product_label} ${record.notes}`))
  ...
  if (hasPercentileRows && (hasSummaryRows || hasRangeOnlyRows)) {
    notes.push({
      ...
      detail: "Some sources provide reconstructable percentiles or raw sample rows, while others provide only mean, median, maximum, or range. Summary-only papers cannot directly produce p90/p10.",
    })
  }
  ...
  if (needsQa) {
    notes.push({
      ...
      detail: "At least one mapped source row has a row-split, OCR, or count-conflict warning. Confirm the source table before promoting it into a final standards calculation.",
    })
  }
    ...
    : [
        {
          ...
          detail: "The extracted rows appear broadly comparable, but human review is still required before final standards selection.",
        },
      ]
}

function regulatoryGuardrailFor(row, metal, candidateValue, candidateUnit, regulations) {
  const linkedIds = new Set(row.applicable_regulations ?? [])
  const linkedRegulations = regulations.filter((regulation) => linkedIds.has(regulation.rule_id) || linkedIds.has(regulation.slug))
  const linkedForMetal = linkedRegulations.filter((regulation) => metalMatches(regulation.metal, metal))
  const applicableLimits = linkedForMetal.filter(
    (regulation) => regulation.status === "finalized" && regulation.limit_value !== null && regulation.limit_unit,
  )
  const strictestLimit = [...applicableLimits].sort((a, b) => (a.limit_value ?? Infinity) - (b.limit_value ?? Infinity))[0] ?? null
  const excludedTracks = excludedRegulatoryTracksFor(row, metal, regulations)

  if (strictestLimit) {
    const sameUnit = !candidateUnit || strictestLimit.limit_unit === candidateUnit
    const status = candidateValue === null ? "candidate_missing" : !sameUnit ? "unit_review" : candidateValue <= strictestLimit.limit_value ? "within_ceiling" : "exceeds_ceiling"
    ...
    return {
      ...
      review_prompts: regulatoryReviewPrompts(row, metal, strictestLimit, status),
    }
  }

  const status = excludedTracks.length > 0 ? "excluded_from_ingested_program" : "no_ceiling_recorded"
  return {
    status,
    status_label: regulatoryStatusLabel(status),
    note:
...
function excludedRegulatoryTracksFor(row, metal, regulations) {
  ...
  const ctzProgram = regulations.find((regulation) => regulation.rule_id === "fda-closer-to-zero")
...
function ctzExclusionReason(rowSlug) {
  if (rowSlug.startsWith("infant-formula")) return "The ingested Closer to Zero overview excludes infant formula from the covered processed-baby-food action levels."
  if (rowSlug === "fruit-juice-not-canned") return "The ingested Closer to Zero overview excludes juices and notes that juices are handled on a separate guidance track."
  if (rowSlug.startsWith("teething-and-snacks")) return "The ingested Closer to Zero overview excludes snack foods, including puffs, rusks, teething biscuits, and grain-based snacks."
  return null
}
...
function blockersFor(row, records, selected, standardBasis, cleanRecord, dirtyRecord, samples, regulatoryGuardrail) {
  const blockers = []

  if (records.length === 0) blockers.push("No row-metal evidence extracted yet.")
  if (!selected) blockers.push(`No qualifying ${standardBasis.statistic_label} distribution selected for this row's standard basis.`)
  if (selected && String(selected.review_state ?? "").includes("machine")) blockers.push("Candidate is machine-extracted and needs human review.")
  if (selected && (numberOrNull(selected.n) ?? 0) < 30) blockers.push("Sample size is below 30; percentile stability needs review.")
  if (selected && samples.length === 0) blockers.push("No sample-level rows linked for confidence or p-value work.")
  if (records.length > 0 && new Set(records.map((record) => record.basis).filter(Boolean)).size > 1) {
    blockers.push("Multiple matrix bases are present; basis normalization/adjudication is needed.")
  }
  if (standardBasis.role === "contamination_platform" && !cleanRecord) {
    blockers.push("Paired clean-category p90 is not available for separation/context.")
  }
  if (standardBasis.role === "clean_benchmark" && !dirtyRecord && row.partners.some((partner) => partner.role_of_partner === "contamination_platform")) {
    blockers.push("Paired dirty-category p10 is not available for separation/context.")
  }
  if (regulatoryGuardrail?.status === "exceeds_ceiling") {
    blockers.push("Candidate exceeds an ingested applicable federal regulatory ceiling; final standard must be at or below that ceiling.")
  }
  if (regulatoryGuardrail?.status === "unit_review") {
    blockers.push("Applicable regulatory ceiling uses a different unit/basis; convert and adjudicate before finalization.")
  }
  if (["no_ceiling_recorded", "excluded_from_ingested_program"].includes(regulatoryGuardrail?.status)) {
    blockers.push("No applicable federal concentration ceiling is linked for this row-metal; regulatory review is required before finalization.")
  }
  if (!selected) blockers.push("Path B LOQ package is needed if the row-standard statistic remains unavailable.")
...
function nextAction(status, blockers) {
  if (status === "finalized") return "Finalized standard recorded; monitor for superseding evidence."
  if (status === "missing") return "Find row-fit occurrence evidence."
  if (status === "path-b") return "Find the row-standard distribution or build LOQ fallback package."
  if (blockers.some((blocker) => blocker.includes("human review"))) return "Human-review machine extraction and source fit."
  if (blockers.some((blocker) => blocker.includes("confidence"))) return "Run confidence and p-value work from sample-level data."
  if (blockers.some((blocker) => blocker.includes("basis"))) return "Adjudicate matrix basis before threshold math."
...
function promptsFor(row, metal, records, blockers, standardBasis) {
  ...
  const sourceIds = [...new Set(records.map((record) => record.source_id))].filter(Boolean).join(", ") || "none yet"
...
async function sourceTitle(sourceId, cache) {
  ...
  try {
    const source = await readFile(resolve(repoRoot, "wiki/sources", `${sourceId}.md`), "utf8")
...
function keyByCell(rows) {
  ...
  for (const row of rows) {
    if (row.row_slug && row.metal_species) map.set(cellKey(row.row_slug, row.metal_species), row)
  }
  return map
}
