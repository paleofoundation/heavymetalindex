...
The `CC candidate summary` section is the only public product-page section that should display the row-standard percentile for threshold math. It has one row per subcategory x HMT&C analyte cell and mirrors the Step 0F CC Source Data Package: clean benchmark rows use the 90th percentile of the selected clean-platform distribution; contaminated-platform rows use the governance-selected lower-tail percentile of the selected contaminated-platform distribution, P10 by default or P20 only when explicitly selected; Path B uses 5x LOQ when Path A data is insufficient. If the CC candidate has not been computed or validated yet, say so rather than displaying a provisional-looking number.

The `How standards math uses this page` section explains that the HMTc row-standard percentile is an aggregate or selected-dataset value, not a per-source decoration. For lower-is-better concentration data, p100/max in a source evidence row means the highest observed concentration in that source's scoped pool. Clean-row p90 means the cutoff at which roughly 90 percent of the selected clean-platform distribution would pass; the dirty-row lower-tail target means the lower tail of the contaminated-platform distribution used to separate the dirty category from the clean platform. Dirty-category p90 and clean-category p10 are not HMTc limit-setting targets. The row-standard percentile belongs in the CC candidate package after inclusion/exclusion and scoping decisions are made, and the final HMTc value must not exceed the lowest applicable loaded regulatory cap.
