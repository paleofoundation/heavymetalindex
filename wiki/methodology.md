...
The Index now uses an Evidence Fitness layer between source extraction and public synthesis. Evidence Fitness answers a narrow question: for this exact metal species, matrix, product type, statistic, unit, and source, what can this record support?
...
| EF-3 | Modeled or limited evidence | The source provides means, ranges, maxima, medians, source-level summaries, or other limited statistics. |
...
The first structured evidence build is tracked under `data/evidence/`. The private `raw/markdown/` corpus remains excluded from git; tracked evidence records contain source handles, candidate values, review states, public labels, and provenance fields only.
...
`approved_for_public` records are required before a value or claim appears on a public Index page as an affirmative evidence statement.

Public pages may show data gaps and review status. They may not present machine-extracted concentration values as settled findings.
...
The pilot register starts with every row-metal cell marked as `Data gap` until approved source evidence is attached. This prevents the public Index and downstream standards work from treating missing data as evidence of low contamination.
